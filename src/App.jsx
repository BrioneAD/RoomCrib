...
// 📷 Lector QR (carga diferida para evitar romper si no hay cámara/permisos)
...
export default function App() {
  ...
  // ✅ Estados del escáner QR (evitan "scannerOpen is not defined")
  const [scannerOpen, setScannerOpen] = useState(false)
  const [cameraFacing, setCameraFacing] = useState('environment') // 'user' para frontal
  ...
  const getProgress = (locItem) => {
    ...
    if (!m || !m.ideal) return 0
    return Math.min(100, Math.max(0, Math.round((Number(locItem.current) / Number(m.ideal)) * 100)))
  }

  const getStockColor = (locItem) => {
    const m = getMaterial(locItem.materialId)
    if (!m || !m.ideal) return 'text-white'
    const ratio = Number(locItem.current) / Number(m.ideal)
  ...
  const findMaterialLocations = (materialId) => {
    return Object.entries(storage)
      .filter(([, items]) => (items ?? []).some((i) => i.materialId === materialId))
      .map(([cab]) => cab)
  }
  ...
  // ---- Cloud (Firebase) ----
  const sendScanToCloud = async (payload) => {
    try {
      if (!db) return // si no hay Firestore inicializado, no enviar
      await addDoc(collection(db, 'roomcrib_scans'), {
        ...payload,
        at: serverTimestamp(),
      })
    } catch (err) {
      console.error('No se pudo enviar a Firebase:', err)
    }
  }

  // ---- QR handler ----
  const onQrUpdate = (err, result) => {
    if (err) {
    ...
    // Bibliotecas diferentes exponen texto de formas distintas
    const raw = typeof result?.getText === 'function' ? result.getText() : (result?.text ?? String(result))
    const text = String(raw).trim().toUpperCase()

    // Acepta formatos como "CRIB:NEGRO-1" o solo "NEGRO-1"
    const candidate = text.includes(':') ? text.split(':').pop() : text
    const allLocations = Object.keys(storage)
    const matchedLoc = allLocations.find((id) => id.toUpperCase() === candidate)

    if (matchedLoc) {
      setSelectedLocation(matchedLoc)
      setSearchTerm(matchedLoc)
      setScannerOpen(false)
      addLog(`QR Detectado: ${matchedLoc}`, 'info')
      sendScanToCloud({ kind: 'location', value: matchedLoc, raw: text })
      return
    }

    // Soporta códigos de materiales tipo MAT-0001
    const maybeMat = text.startsWith('MAT-') ? text : null
    if (maybeMat && materials[maybeMat]) {
      setSearchTerm(maybeMat)
      setScannerOpen(false)
      addLog(`QR de material: ${maybeMat}`, 'info')
      sendScanToCloud({ kind: 'material', value: maybeMat, raw: text })
      return
    }

    addLog(`QR no reconocido: ${text}`, 'info')
  }
  ...
  const handleScanSimulation = () => {
    ...
    setTimeout(() => {
      ...
      // También simulamos un envío a la nube
      sendScanToCloud({ kind: 'location', value: randomLoc, raw: 'SIMULATED' })
    }, 1000)
  }
  ...
  // 7) Render
  // -----------------------------
  const canUseCamera = typeof navigator !== 'undefined' && !!navigator.mediaDevices
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto">
        ...
        {scannerOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4">
            <div className="bg-gray-900 rounded-2xl p-4 w-[92vw] max-w-md border border-yellow-400/20 neon-border-strong">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-black text-yellow-400 uppercase text-xs tracking-widest">Escanear QR</h4>
                <button onClick={() => setScannerOpen(false)} className="p-1 rounded hover:bg-white/5">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="rounded-xl overflow-hidden border border-white/10 bg-black">
                {canUseCamera ? (
                  <Suspense fallback={<div className="p-6 text-sm text-gray-400">Cargando cámara…</div>}>
                  ...
                  <div className="p-6 text-sm text-amber-300">
                    Cámara no disponible en este entorno. Prueba en HTTPS y otorga permisos.
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between mt-3 gap-2">
                <button
                  onClick={() => setCameraFacing((c) => (c === 'environment' ? 'user' : 'environment'))}
                  className="text-xs px-3 py-1 bg-white/5 border border-white/10 rounded"
                >
                  Cambiar cámara
                </button>
                <button
                    ...
                    {filteredCabinets.map((id) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center justify-center h-full relative z-[1]">
                          <span className="font-black text-3xl text-white drop-shadow">
                            {(storage[id] ?? []).length}
                          </span>
                          <span className="text-[9px] uppercase font-bold tracking-widest mt-1 opacity-70">
                            Materiales
                          </span>
                        </div>
                  ...
                  <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
                    {(storage[selectedLocation] ?? []).length > 0 ? (
                      (storage[selectedLocation] ?? []).map((locItem) => {
                        ...
                        return (
                          <div
                            key={`${selectedLocation}-${locItem.materialId}`}
                            className="rounded-xl border border-yellow-400/10 bg-black/40 p-4 group"
                          >
                            <div className="flex justify-between mb-2">
                              <div>
                                <h4 className="font-black text-yellow-400 text-sm uppercase">
                                  {m?.name ?? locItem.materialId}
                                </h4>
                                <p className="text-[10px] text-gray-500">ID: {m?.id ?? locItem.materialId}</p>
                              </div>
                              <button
                                onClick={() => deleteMaterialFromCabinet(selectedLocation, locItem.materialId)}
                            ...
                            <div className="flex items-end justify-between mb-3">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <input
                                  ...
                                  value={getMaterial(locItem.materialId)?.ideal ?? 0}
                ...
                <div className="p-6">
                  ...
                  {term && materialMatches.length > 0 ? (
                    ...
                    <div className="h-64 flex flex-col items-center justify-center opacity-80">
                      ...
                      <p className="font-black uppercase text-[10px] tracking-[0.4em] mt-4 text-gray-300">
                        RoomCRIB Ready
                      </p>
                    </div>
                  )}
